...
struct ConfigItem {
	string name;  // 项目名
	string value; // 项目值
	string rawline;  //原本的组名
};

// 配置组结构体
struct ConfigGroup {
	string name;                // 组名
	vector<ConfigItem> items;   // 当前组的配置项
};
...
private:
	...
	bool check_open = false;//标记是否打开成功
	vector<ConfigGroup> cfg_list;//存每一行个

	ConfigGroup* find_group(const std::string& group_name, const bool is_case_sensitive);
	ConfigItem* find_item(ConfigGroup* group, const std::string& item_name, const bool is_case_sensitive);
	
public:
	/* 构造函数的作用：将指定配置文件中的所有组/项均读入并存储到自定义结构中，方便后续各种get函数的调用 */
	config_file_tools(const char* const _cfgname, const enum BREAK_CTYPE bctype = BREAK_CTYPE::Equal);
	config_file_tools(const string& _cfgname, const enum BREAK_CTYPE bctype = BREAK_CTYPE::Equal);
